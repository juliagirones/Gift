// script.js — Black Hollow Episodios 1 & 2 (versión final, respeta tu guion)
...
const htmlBgAudio = document.getElementById('bgAudio'); // opcional, no usado para ambient por defecto
...
let state = {
  ...
  flags: {
    ...
    angieAlly: false,
    skittlesPath: false,
    oldWomanKindness: false,
    monsterTheory: null, // 'skinwalker' | 'wendigo'
    factoryClueLost: false
  }
};

const SAVE_KEY = 'bh_ep12_vfinal';
...
function cutToBlack(onMid){
  ...
  setTimeout(()=>{
    if(onMid) try{ onMid(); }catch(e){}
    blackFade.style.opacity = 0;
  }, 700);
}

// Máquina de escribir lenta
...
function clearChoices(){
  ...
  continueBtnInGame.textContent = 'Continuar';
}
...
// ---------------- Ambient (WebAudio) — brown noise base + pads crossfade
const Ambient = (function(){
  let ctx, master, noiseNode, noiseGain, busA, busB, currentBusIndex = 0;
  ...
  function ensureCtx(){
    if(!ctx){
      ...
      // Brown noise base
      noiseGain = ctx.createGain(); noiseGain.gain.value = 0.08;
      noiseNode = makeBrownNoise();
      noiseNode.loop = true;
      noiseNode.connect(noiseGain); noiseGain.connect(master);

      // buses for pads crossfade
      busA = ctx.createGain(); busB = ctx.createGain();
      busA.gain.value = 0.0; busB.gain.value = 0.0;
  ...
  // builder de pad sencillo (dos osciladores + filtro)
  function buildPad({freq=60, detune=3, type='sine', lp=900}){
    const padGain = ctx.createGain(); padGain.gain.value = 0.0;
    const o1 = ctx.createOscillator(); o1.type = type; o1.frequency.value = freq; o1.detune.value = -detune;
    const o2 = ctx.createOscillator(); o2.type = type; o2.frequency.value = freq * 1.997; o2.detune.value = detune;
    const g1 = ctx.createGain(); g1.gain.value = 0.25;
    const g2 = ctx.createGain(); g2.gain.value = 0.21;
    o1.connect(g1); o2.connect(g2); g1.connect(padGain); g2.connect(padGain);

    const lpFilter = ctx.createBiquadFilter(); lpFilter.type='lowpass'; lpFilter.frequency.value = lp;
    padGain.connect(lpFilter);

    o1.start(); o2.start();
    return { out: lpFilter, control: padGain };
  }

  const scenes = {
    wind_soft: ()=> buildPad({freq:72, type:'sine', lp:950}),
    forest: ()=> buildPad({freq:88, type:'triangle', lp:1000}),
    forest_quiet: ()=> buildPad({freq:68, type:'sine', lp:800}),
    cabin: ()=> buildPad({freq:55, type:'sine', lp:700}),
    factory: ()=> buildPad({freq:95, type:'triangle', lp:1100}),
    cave_breath: ()=> buildPad({freq:48, type:'sine', lp:600}),
    cave_pulse: ()=> buildPad({freq:52, type:'triangle', lp:600}),
    tension: ()=> buildPad({freq:76, type:'sine', lp:850}),
    bar_night: ()=> buildPad({freq:120, type:'sine', lp:1200}),
    low_boom: ()=> buildPad({freq:34, type:'sine', lp:500})
  };

  // crossfade to scene pad
  function crossfadeTo(key){
    ensureCtx();
    if(!noiseNode.started){ try{ noiseNode.start(); noiseNode.started = true; }catch(e){} }
    const builder = scenes[key] || scenes.wind_soft;
    const pad = builder(); // {out, control}
    const useA = currentBusIndex === 1; // alternate which bus we attach to
    const targetBus = useA ? busA : busB;
    const otherBus = useA ? busB : busA;

    pad.out.connect(targetBus);
    // ramp up target bus and ramp down other bus
    const now = ctx.currentTime;
    targetBus.gain.cancelScheduledValues(now); otherBus.gain.cancelScheduledValues(now);
    targetBus.gain.setValueAtTime(0.0, now);
    targetBus.gain.linearRampToValueAtTime(0.8, now + 1.1);
    otherBus.gain.setValueAtTime(otherBus.gain.value, now);
    otherBus.gain.linearRampToValueAtTime(0.0, now + 1.0);

    // also slowly ramp pad control in
    pad.control.gain.setValueAtTime(0.0, now);
    pad.control.gain.linearRampToValueAtTime(1.0, now + 1.0);

    currentBusIndex = useA ? 0 : 1;
  }

  function resume(){
    ensureCtx();
    if(ctx.state !== 'running') ctx.resume();
    // start noise if not started
    if(noiseNode && !noiseNode.started){
      try{ noiseNode.start(); noiseNode.started = true; }catch(e){}
    }
  }

  function whisper(freq = 1600){
    ensureCtx();
    const g = ctx.createGain(); g.gain.value = 0.0; g.connect(master);
    const n = makeBrownNoise();
    const bp = ctx.createBiquadFilter(); bp.type='bandpass'; bp.frequency.value = freq; bp.Q = 4;
    n.connect(bp); bp.connect(g);
    n.start();
    const now = ctx.currentTime;
    g.gain.linearRampToValueAtTime(0.28, now + 0.12);
    g.gain.linearRampToValueAtTime(0.0, now + 1.1);
    setTimeout(()=>{ try{ n.stop(); }catch(_){} }, 1200);
  }
  ...
  function stinger(){
    ...
    const now = ctx.currentTime;
    g.gain.linearRampToValueAtTime(0.85, now + 0.05);
...
// ------------------ Nodos del guion (texto EXACTO que nos diste)
// Todos los textos se almacenan como strings; algunas escenas usan textFn(state) para texto dinámico.
const NODES = [
  // ---------- EPISODIO 1 ----------
...
`Elias (bajo):
“Estuvieron husmeando en la caverna, con tal de encontrar cualquier cosa que puedan vender...oro, plata...encontraron pinturas y... Algo antiguo. Algo hambriento. Escucha a las hojas. Ellas susurran la verdad.”`, next:'S2_LIBRETA' },
...
Mujer con sombrero y canasta pasa sin detenerse. Ojos apenas visibles, brillos blancos.
...
`Mujer:
“Eso dijeron los últimos; que venían a investigar las cavernas. A buscar oro, plata... Pero tu escuchame bien… si oyes ruidos que imitan voces en el bosque: no respondas, no los escuchaste, no corras, no te escondas, no pares. Date la vuelta y regresa.”`, next:'S4_BOSQUE' },
...
`En ese momento, una chica aparece corriendo desde el fondo, asustada. Se tropieza. Es Angie, pálida y sin zapatos.
Angie (temblando):
...
Entrega una página rasgada de un diario con símbolos dibujados: parecen coincidir con los de las cavernas.`, next:'S5_SENDEROS' },
...
`La vegetación es más densa. El ambiente está cada vez más silente, como si los sonidos naturales se hubieran suspendido. El viento sopla con un murmullo extraño. Metu detiene su paso, inquieta. Mira hacia atrás. El camino ya no es tan claro.
...
"¿Y si no puedo volver...?"`,
  ...
  { id:'S6_CAVERNA', sound:'cave_breath', onEnter: ()=> Ambient.whisper(1400), text:
`Atardecer profundo. El cielo se tiñe de morado y rojo. La brisa del bosque se vuelve helada, como si algo estuviera conteniendo el aire. Metu, ahora sola en un sendero apenas visible, se detiene. La vegetación se abre paso a una caverna oscura, irregular y cubierta de raíces como venas petrificadas. De su interior emana un sonido sutil, húmedo… como si algo respirara.

VOZ (susurrando):
“Metu...”`, next:'S6_VOZ2' },

  { id:'S6_VOZ2', sound:'cave_pulse', onEnter: ()=> Ambient.whisper(1800), text:
...
No se mueve. No respira. Solo observa. Un parpadeo… y desaparece.`, next:'OUTRO_EP1' },
  ...
  { id:'EP2_INTRO', sound:'tension', textFn:(st)=>{
      const elias = st.flags.eliasDistrust ? 'Elías no confía en ti.' : 'Elías confía en ti.';
      const sk = st.flags.skittlesPath ? 'Dejaste Skittles marcando el camino.' : 'No dejaste rastro alguno en el bosque.';
...
`[Encuentras la Fábrica]. “Es una fábrica de Tabaco…bueno, era una fábrica de Tabaco. Está muy cerca de la caverna, creo…debe estar por aquí”. [El viento sopla helado, seco. Te acercas a la ventana y logras observar que este es el primer piso. Hay tiendas de campaña adentro].`,
    choices:[
...
[Ves al suelo, una última mirada antes de salir…Un Diario.]`, next:'E2_S1_A_DIARIO1' },
...
[El resto era ilegible, páginas arrancadas o llenas de sangre. Había escrito más, pero era imposible seguir leyendo]`, next:'E2_S2' },

  { id:'E2_S1_B', sound:'factory', onEnter: ()=>{ state.flags.factoryClueLost = true; save(); }, text:`Pierdes una pista por miedo a entrar.`, next:'E2_S2' },

  // ESCENA 2
  { id:'E2_S2', sound:'cabin', text:
`[En la cabaña] Metu: “Qué habrán sido esos símbolos raros en las paredes de la fábrica…” [Notas que te sientes observada.] “Debería ir a preguntarle a Elías…él me dijo que no fuera a la fábrica. Él sabe lo que pasó”.`, next:'E2_S3' },

  // ESCENA 3 (Elías, branch por flag)
  { id:'E2_S3', sound:'cabin', textFn:(st)=>{
...
Algo como lo que ella contaba, no debería de salir nunca de la cueva. Pero como te dije la primera vez que te vi, chica de ciudad. Lo despertaron. Lo sacaron y ahora, esa cosa sabe venir aquí, sabe salir del bosque.”`;
  ...
  // ESCENA 4 — Señora y perro
  { id:'E2_S4', sound:'cabin', text:
...
Señora: “Hija. Estás en peligro. Tu y tu amigo deben irse. Tu no sabes a lo que te enfrentas.”`,
    choices:[
      { label:'“¡Váyase de aquí! ¿Me ha estado siguiendo? ¡Es por usted que me he sentido así, perseguida y acosada! Por favor déjeme en paz”', next:'E2_S4_A' },
      { label:'“¿Amigo? ¿Cuál amigo? Por favor pase…”', setFlag:{ key:'oldWomanKindness', value:true }, next:'E2_S4_B' }
    ]
  },
  { id:'E2_S4_A', sound:'cabin', text:`“¡Te he seguido desde que saliste de la casa de Elías! ¡Están en peligro!” [Metu cierra la puerta]`, next:'E2_S5' },
  { id:'E2_S4_B', sound:'cabin', onEnter: ()=>{ notify('La señora recordará tu amabilidad.'); }, text:`Señora: “El investigador, hija. Tu amigo. ¿No vino contigo? Vino haciendo las mismas preguntas que tú. Ayer estuvo en la caverna y se cortó. Ambos están en peligro. Vine a advertirte.”`, next:'E2_S5' },

  // ESCENA 5 — Investigar al investigador o ir a la caverna
...
`[Policía en la escena del hotel. La cama deshecha, sangre en el suelo; el marco con restos de uñas.]
Policía: “Por favor salga del perímetro. No vamos a contestar preguntas.”`, next:'E2_S6' },
  { id:'E2_S5_B', sound:'tension', text:
`[Caminas por el pueblo, decidida a entrar al bosque. Elías en su mecedora te observa…]
Elías: “Se lo llevó…Deberías de irte. No va a detenerse. Si las leyendas son ciertas, él olió su sangre y por eso se lo llevó.”`, next:'E2_S6' },
  ...
  // ESCENA 6 — Skittles flag
  { id:'E2_S6', sound:'forest', textFn:(st)=>{
...
Metu: “No perderé el tiempo entonces. Excelente”`;
  ...
  // ESCENA 7 — Entrada de la caverna y Angie flag
...
[El olor es fuerte, es innegable y te golpea justo en la cara. Un olor putrefacto, húmedo, denso, como si el aire adentro desconociera del frío silencioso del bosque]`, next:'E2_S7_FLAG' },
  ...
  { id:'E2_S7_FLAG', sound:'cave_pulse', textFn:(st)=>{
...
Metu: Es eso…¡Es eso! Se trata de un…`,
    choices:[
      { label:'¡Un Skinwalker!', setFlag:{ key:'monsterTheory', value:'skinwalker' }, note:'Has decidido que se trata de un Skinwalker.', next:'E2_END_A' },
      { label:'¡Un Wendigo!', setFlag:{ key:'monsterTheory', value:'wendigo' }, note:'Has decidido que se trata de un Wendigo.', next:'E2_END_B' }
    ]
  },

  { id:'E2_END_A', sound:'tension', text:`Metu: “Si es así, esta noche, se llevará a alguien más…”`, next:'E2_FINAL_MSG' },
  { id:'E2_END_B', sound:'tension', text:`Metu: “Si es así, esta noche, se llevará a alguien más…”`, next:'E2_FINAL_MSG' },

  // Mensaje final (antes del To be continued final separado)
  { id:'E2_FINAL_MSG', sound:'low_boom', text:`“Más vale que tengas razón.”`, next:'E2_FINAL_TO_BE' },

  // Último To be continued (separado)
  { id:'E2_FINAL_TO_BE', sound:'low_boom', text:`\n\nTo be continued...`, end:true }
];
...
async function showNode(id){
  ...
  const node = NODE[id];
  if(!node) return endToMenu();

  // cinematic: fade to black then set ambient
  cutToBlack(()=>{
    clearChoices();
    storyText.textContent = '';
  ...
  if(node.textFn && typeof node.textFn === 'function'){
    try{ textToShow = node.textFn(state); }catch(e){ textToShow = node.text || ''; }
  }

  // call onEnter if exists
  if(node.onEnter && typeof node.onEnter === 'function'){
    try{ node.onEnter(); }catch(e){}
  }
  ...
  if(node.choices && node.choices.length){
    ...
    // show "Siguiente" button to go to Episode 2 title
    await wait(220);
    showContinue(()=> goNext(node), 'Siguiente');
  } else if(node.end){
    // final node: wait a little and return to menu
    await wait(1400);
    endToMenu();
  } else {
    await wait(260);
...
function handleChoice(choice){
  ...
  setTimeout(()=> showNode(choice.next), 720);
}
